...
function formatJson(data, parentSymbol) {
	...
	xmlhttp.onreadystatechange = function()
  	{
  		...
  		if ((xmlhttp.readyState == 4) && (xmlhttp.status == 200))
    	{
			...
			//keep regular formatting in JSON data returned from YAHOO API while still in string format
			if (data.search('"quote":{"symbol"') != -1){
				data = data.replace('"quote":{"symbol"', '"quote":[{"symbol"');
				data = data.replace('}}}}', '}]}}}');
			}
			
			//change data from string to JSON object
    		data = jQuery.parseJSON(data || "null"); 
			
			//"break" data branch off at workable quote data
    		data = breakJsonBranch(breakJsonBranch(breakJsonBranch(data,'query'),'results'),'quote');
    
    		//if error free then output unordered list to desired div element
    		if (data != null) {
    			document.getElementById(divName).innerHTML = formatJson(data);
    		}
    	}
  	}
...
$(document).ready(function () {
	...
	$("#quoteData").prepend("<input type='text' size='15' class='stockSymbol' /><input type='button' value='+' class='qtyplus' '/>");
